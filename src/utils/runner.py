...
#相关的方法已经整合过了，按照下面的来
from utils.hand_eval_utils import (
    EvalUtil,
    rigid_align,
    mano_to_mpii,
    mpii_to_mano,
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for step, data in enumerate(self.val_loader):
                ...
                # 获取verts的ground truth
                if 'verts' in data:
                    verts_gt = data['verts'][0].cpu().numpy() * 0.2  # [778, 3]
                    joint_cam_gt = data['joint_cam'][0].cpu().numpy() * 1000.0
                    
                    # 计算vertex errors
                    verts_pred_mm = verts_pred * 1000.0  # 转换为mm，假设模型输出单位为米
                    verts_gt_mm = verts_gt * 1000.0      # 转换为mm
                    
                    # MPVPE (Mean Per Vertex Position Error)
                    verts_error = np.sqrt(np.sum((verts_pred_mm - verts_gt_mm) ** 2, axis=1))
                    verts_errors.append(verts_error)
                    
                    # PA-MPVPE (Procrustes Aligned MPVPE)
                    verts_aligned_mm = self.rigid_align_vertices(verts_pred_mm, verts_gt_mm)
                    pa_verts_error = np.sqrt(np.sum((verts_aligned_mm - verts_gt_mm) ** 2, axis=1))
                    pa_verts_errors.append(pa_verts_error)
                    
                    # F@5mm和F@15mm (Fraction of vertices within threshold)
                    f_score_5mm = np.mean(verts_error < 5.0)   # 5mm threshold
                    f_score_15mm = np.mean(verts_error < 15.0) # 15mm threshold
                    verts_f_scores_5mm.append(f_score_5mm)
                    verts_f_scores_15mm.append(f_score_15mm)
            ...
            # 计算新的vertex指标
            if verts_errors:
                mpvpe = np.concatenate(verts_errors).mean()
                pa_mpvpe = np.concatenate(pa_verts_errors).mean()
                f_score_5mm = np.array(verts_f_scores_5mm).mean()
                f_score_15mm = np.array(verts_f_scores_15mm).mean()
